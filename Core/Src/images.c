...
#define errorDownload     0,  (0+hh),    465, 150
...
uint8_t buff_old[21]; //Буфер экрана старые значения
uint8_t buff_new[21]; //Буфер экрана новый
...
//---------------------Тампература платы новая--------------------------------------
 ...
 uint8_t error = 0;//ошибки

 volatile static uint8_t var_off = 0; //Флаг режима Off
 volatile uint8_t dots_img = 0; //Флаг включения часов
 volatile static uint8_t cool_img = 0; //Флаг включения кулера
 volatile static uint8_t pass_startTim2 = 0;//Флаг старта TIM2
 volatile uint8_t is_cooler_mode = 0;//Флаг старта кулера
 volatile uint8_t is_dots_mode = 0;//Флаг старта мигания точек
 volatile uint8_t isTurnOff = 0;//Флаг режима Off
...
void turn_off(){
	if (new_set == 0 && var_off == 0){//Если через 10 сек режим "Off" не был выключен(проверяем для исключения ненужных переходов в этот режим)
		...
		is_dots_mode = 0;
		drawImage(dots_off, 365, (63 + hh), 5, 25);
...
const char* error_msgs[] = {
    "",                              // 0 - нет ошибки
    "ОБНАРУЖЕН ОБРЫВ НИЖНЕГО ТЭНА",           // 1
    "ОБНАРУЖЕН ОБРЫВ ВЕРХНЕГО ТЭНА",          // 2
    "ОБНАРУЖЕН ОБРЫВ БОКОВОГО ТЭНА",              // 3
    "ОБНАРУЖЕНА  АВАРИЯ НИЖНЕГО ТЭНА",          // 4
    "ОБНАРУЖЕНА  АВАРИЯ ВЕРХНЕГО ТЭНА",     // 5
    "ОБНАРУЖЕНА  АВАРИЯ БОКОВОГО ТЭНА",           // 6
    "ОБНАРУЖЕНО ВКЛЮЧЕНИЕ ВСЕХ ТЭНОВ"                 // 7
};

void check_images() {
	// 1. Сначала определяем, должны ли мы сейчас принудительно показывать ошибку
	static uint8_t last_force_state = 0; // Храним предыдущее состояние (0-норма, 1-ошибка)
	    uint8_t force_error_screen = 0;
	    uint32_t sec = HAL_GetTick() / 1000; // получаем секунды

	    if (error >= 4 && error <= 7) {
	        // Фатальные ошибки - показываем всегда
	        force_error_screen = 1;
	    }
	    else if (error >= 1 && error <= 3) {
	        // Нефатальные: цикл 90 секунд (60 сек норма + 30 сек ошибка)
	        // Каждую минуту (с 0 по 60 сек) - норма, с 60 по 90 сек - ошибка
	        if ((sec % 90) >= 60) {
	            force_error_screen = 1;
	        }
	    }

	    // 2. ЛОГИКА ПЕРЕРИСОВКИ ПРИ ВОЗВРАТЕ
	        // Если в прошлом цикле была Ошибка (1), а сейчас Норма (0)
	        if (last_force_state == 1 && force_error_screen == 0) {
	            fillScreen(0x0000); // Чистим экран перед возвратом
	            screen_first();     // Рисуем сетку, рамки и статику основного экрана
	            var_off = 0;        // Сбрасываем флаг Off, чтобы всё отрисовалось заново
	        }
	        last_force_state = force_error_screen; // Запоминаем текущее состояние для следующего цикла

	    // 2. Основная развилка отрисовки
	    if (error == 0 || (force_error_screen == 0)) {
	if (new_set > 10){
	...
	if (new_set != 0){      //Если новый 11 член массива массива не равен "0"
		...
		if (var_off == 1){ //Если быстро крутили и повар не стерся
			screen_first(); //Рисуем начальный экран режимов
...
//---------------------------------------------------------------------------------------------
	...
	//HAL_Delay(2000);
		...
		// --- ЭКРАН ОШИБКИ (активен для 4-7 всегда, для 1-3 по таймеру) ---
		        fillScreen(0x0000);

		        // Рисуем рамку
		        fillRect(0, hh, 465, 2, ILI9488_CYAN);
		        fillRect(0, 148 + hh, 465, 2, ILI9488_CYAN);
		        fillRect(0, 2 + hh, 2, 148, ILI9488_CYAN);
		        fillRect(463, 2 + hh, 2, 148, ILI9488_CYAN);

		        // Картинка "ВНИМАНИЕ"
		        drawImage(vnimanie, 170, 8 + hh, 188, 23);//Рисуем картинку с красным текстом "ВНИМАНИЕ"

		        // Текст ошибки (выбираем нужную строку из массива)
		        TFT_WriteString(30, 60 + hh, error_msgs[error], Font_11x18, ILI9488_WHITE, ILI9488_BLACK);
		    }
}
